...
import com.chan.chandummy.customview.CustomTabLayout.Tab
 ...
 * done through [Tab] instances. You create tabs via [.newTab]. From there you can
 * change the tab's label or icon via [Tab.setText] and [Tab.setIcon]
 * respectively. To display the tab, you need to add it to the layout via one of the
 * [.addTab] methods. For example:
 * <pre>
 * TabLayout tabLayout = ...;
 ...
 *
 *
 * If you're using a [android.support.v4.view.ViewPager] together
 * with this layout, you can call [.setupWithViewPager] to link the two together.
 ...
 * directly to the ViewPager in a layout resource file like so:
 *
 * <pre>
 ...
 */
@ViewPager.DecorView
class CustomTabLayout @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) : HorizontalScrollView(context, attrs, defStyleAttr) {
    ...
    init {
        ...
        super.addView(mTabStrip, 0, LayoutParams(LayoutParams.WRAP_CONTENT, LayoutParams.MATCH_PARENT))

        //Custom strip width
        val customStyle = context.obtainStyledAttributes(attrs, com.chan.chandummy.R.styleable.CustomTabLayout,
                defStyleAttr, R.style.Widget_Design_TabLayout)

        mTabStrip.setSelectedIndicatorWidth(
                customStyle.getDimensionPixelSize(com.chan.chandummy.R.styleable.CustomTabLayout_tabIndicatorWidth, 0))
        customStyle.recycle()
    ...
    /**
     * Remove all previously added [OnTabSelectedListener]s.
     */
    ...
    private fun ensureScrollAnimator() {
        if (mScrollAnimator == null) {
            ...
            mScrollAnimator!!.interpolator = FAST_OUT_SLOW_IN_INTERPOLATOR
    ...
    @JvmOverloads
    internal fun selectTab(tab: Tab?, updateIndicator: Boolean = true) {
    ...
    private inner class SlidingTabStrip internal constructor(context: Context) : LinearLayout(context) {
        ...
        private var mSelectedIndicatorWidth: Int = 0
         ...
         * Customized width
         */
        internal fun setSelectedIndicatorWidth(height: Int) {
            if (mSelectedIndicatorWidth != height) {
                mSelectedIndicatorWidth = height
                ViewCompat.postInvalidateOnAnimation(this)
            }
        }
        ...
        /**
         * Sets draws the bottom strip line with custom width and height
         */
        override fun draw(canvas: Canvas) {
            ...
            if (mIndicatorLeft in 0..(mIndicatorRight - 1)) {
                if(mSelectedIndicatorWidth > 0) {
                    val midPoint = mIndicatorLeft + ((mIndicatorRight - mIndicatorLeft)/2)
                    val offsetWidth = mSelectedIndicatorWidth/2
                    val leftPoint = midPoint - offsetWidth
                    val rightPoint = midPoint + offsetWidth
                    canvas.drawRect(leftPoint.toFloat(), (height - mSelectedIndicatorHeight).toFloat(),
                            rightPoint.toFloat(), height.toFloat(), mSelectedIndicatorPaint)
                } else {
                    canvas.drawRect((mIndicatorLeft + 100).toFloat(), (height - mSelectedIndicatorHeight).toFloat(),
                            (mIndicatorRight - 100).toFloat(), height.toFloat(), mSelectedIndicatorPaint)
                }
            }
        }
    }
     ...
     * not cause a leak.
     */
    class TabLayoutOnPageChangeListener(tabLayout: CustomTabLayout) : ViewPager.OnPageChangeListener {
        private val mTabLayoutRef: WeakReference<CustomTabLayout>
